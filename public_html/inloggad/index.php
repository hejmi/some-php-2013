...
//Se om artikel existerar eller finns i lager
	...
	for ($i=0;$i<20;$i++) {
		...
		if ($art_id == "") continue;
		$result = mysql_query("SELECT * FROM products WHERE art_id = '".$art_id."' AND in_stock=1");
...
echo '<br/><span style="font-size:14px;">'.$customer_school_name.'</span></h3>';
if ($customer_is_a_school == 0) {
	echo '<p><a href="?ordercreate">'.$_ORDER_CREATE.'</a> | <!--<a href="?orderoverview">'.$_ORDER_OVERVIEW.'</a> |--> <a href="?logout">'.$_LOG_ME_OUT.'</a></p>';
} else { 
	echo '<p><a href="?pupilsoverview">'.$_PUPILS_OVERVIEW.'</a> | <a href="?logout">'.$_LOG_ME_OUT.'</a></p>';
}
...
<input type="submit" class="greenOKbutton" value="<?php echo $_BUTTON_SAVE; ?>">
</form> 

<?php
	//echo "<p><b>De lökar som fortfarande finns kvar att beställa är;</b></p>";
	//$resultat = mysql_query("SELECT * FROM products ORDER BY art_id");
	//while($row = mysql_fetch_array($resultat)) {
	//	echo $row['art_id'].' - '.$row['art_name_sv'].'</br>';
	//}
}
?>
